...
nnoremap <leader>yi :let @+=expand('%')<CR>:echo 'File path copied to clipboard'<CR>
" write file
...
" indentLine
let g:indentLine_fileType = ['typescript', 'javascript', 'json', 'html']
...
" Get current file's GitHub link with the line number
function! GitHubLink()
  let l:repo_url = system('git config --get remote.origin.url | tr -d "\n"')
  if l:repo_url == ''
  ...
  let l:current_file = expand('%:p')[len(l:git_root) + 1:]
  let l:line_number = line('.')
  let l:github_link = l:repo_url . '/blob/master/' . l:current_file . '#L' . l:line_number

  " Use pbcopy on macOS, xclip on Linux
  if has('macunix')
    let l:copy_cmd = 'echo "' . l:github_link . '" | pbcopy'
  elseif has('unix')
    let l:copy_cmd = 'echo "' . l:github_link . '" | xclip -selection clipboard'
  else
    echo "Clipboard copying not supported on this platform"
    return
  endif

  call system(l:copy_cmd)
  echo "GitHub link copied to the clipboard!"
endfunction

nnoremap <leader>yg :call GitHubLink()<CR>
